...
func (p *parquetInputReader) readFile(
	...
	// TODO: Add StrictMode to ParquetOptions if needed
	consumer, err := newParquetRowConsumer(p.importCtx, producer, fileCtx, false /* strict */)
	if err != nil {
		return err
	}
...
func (p *parquetRowProducer) readBatchFromColumn(
	...
	batch := newParquetColumnBatch(colReader, p.buffers, rowsToRead)
...
func determineColumnsToRead(importCtx *parallelImportContext, parquetSchema *schema.Schema) []int {
...
func validateAndBuildColumnMapping(
	importCtx *parallelImportContext, producer *parquetRowProducer,
...
func (c *parquetRowConsumer) FillDatums(
	ctx context.Context, rowData interface{}, rowNum int64, conv *row.DatumRowConverter,
) error {
