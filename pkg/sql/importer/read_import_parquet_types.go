...
func formatDecimalFromInt32(v int32, scale int32) string {
	// Validate scale to prevent overflow: 10^10 overflows int32
	// For scales > 9, the formatted result would have more digits than the original value,
	// so we just return the raw integer value as a fallback
	if scale > 9 {
		d := apd.New(int64(v), 0)
		return d.Text('f')
	}
...
func formatDecimalFromInt64(v int64, scale int32) string {
	// Validate scale to prevent overflow: 10^19 overflows int64
	// For scales > 18, the formatted result would have more digits than the original value,
	// so we just return the raw integer value as a fallback
	if scale > 18 {
		d := apd.New(v, 0)
		return d.Text('f')
