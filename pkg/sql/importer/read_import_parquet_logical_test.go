...
func TestConvertLogicalValueToDatum(t *testing.T) {
	...
	tests := []struct {
		...
		// Edge cases: overflow protection in decimal formatting
		{
			name:        "int32-decimal-scale-too-large",
			value:       int32(12345),
			targetType:  types.Decimal,
			logicalType: schema.NewDecimalLogicalType(20, 10), // Scale > 9 causes overflow, should fallback
			expected:    func() tree.Datum { d, _ := tree.ParseDDecimal("12345"); return d }(),
		},
		{
			name:        "int64-decimal-scale-too-large",
			value:       int64(123456789),
			targetType:  types.Decimal,
			logicalType: schema.NewDecimalLogicalType(30, 19), // Scale > 18 causes overflow, should fallback
			expected:    func() tree.Datum { d, _ := tree.ParseDDecimal("123456789"); return d }(),
		},
		{
			name:        "int32-decimal-max-safe-scale",
			value:       int32(123456789),
			targetType:  types.Decimal,
			logicalType: schema.NewDecimalLogicalType(10, 9), // Scale = 9 is max safe for int32
			expected:    func() tree.Datum { d, _ := tree.ParseDDecimal("0.123456789"); return d }(),
		},
		{
			name:        "int64-decimal-max-safe-scale",
			value:       int64(123456789012345678),
			targetType:  types.Decimal,
			logicalType: schema.NewDecimalLogicalType(20, 18), // Scale = 18 is max safe for int64
			expected:    func() tree.Datum { d, _ := tree.ParseDDecimal("0.123456789012345678"); return d }(),
		},
