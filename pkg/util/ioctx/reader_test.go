...
func (m *mockReadCloserCtxBrokenSeek) ReadAt(ctx context.Context, p []byte, off int64) (int, error) {
	if off >= int64(len(m.data)) {
		return 0, io.EOF
	}
	n := copy(p, m.data[off:])
...
func (m *mockReadCloserCtxBrokenSeek) Seek(ctx context.Context, offset int64, whence int) (int64, error) {
